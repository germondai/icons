...
function parseViewBox(svg: string): { vw: number; vh: number } | null {
  const parts = svg.match(/viewBox="([^"]+)"/)?.[1]?.trim().split(/\s+/) ?? []
...
function isGradient(f: string): boolean { return f.startsWith("url(") }
...
interface PathShp   { kind: "path";   d: string; fill: string }
interface RectShp   { kind: "rect";   x: string; y: string; w: string; h: string; rx?: string; fill: string }
interface CircleShp { kind: "circle"; cx: string; cy: string; r: string; fill: string }
...
function collectShapes(svg: string, vw: number, vh: number): Shp[] {
  ...
  for (const [, a] of body.matchAll(/<rect([^>]*)(?:\/>|>[\s\S]*?<\/rect>)/gi)) {
    ...
    out.push({
      kind: "rect", fill: getFill(a),
      x: a.match(/\bx="([^"]+)"/)?.[1] ?? "0",
      y: a.match(/\by="([^"]+)"/)?.[1] ?? "0",
      w: dim("width", vw), h: dim("height", vh),
      rx: a.match(/\brx="([^"]+)"/)?.[1],
    })
  }
...
function buildKnockout(svg: string, vw: number, vh: number): string | null {
  ...
  for (const s of shapes) {
    ...
    if (s.kind === "path")
      maskItems.push(`<path fill="black" d="${s.d}"/>`)
    else if (s.kind === "rect")
      maskItems.push(`<rect fill="black" x="${s.x}" y="${s.y}" width="${s.w}" height="${s.h}"/>`)
    else if (s.kind === "circle")
      maskItems.push(`<circle fill="black" cx="${s.cx}" cy="${s.cy}" r="${s.r}"/>`)
  }
...
function buildWhiteKnockout(svg: string, vw: number, vh: number): string | null {
  ...
  const wkoInvisible = (f: string) => { const v = f.toLowerCase().trim(); return v === "none" || v === "transparent" }
  ...
  const setFill = (raw: string, fill: string) =>
    /\bfill="/.test(raw) ? raw.replace(/\bfill="[^"]*"/, `fill="${fill}"`)
  ...
  for (let i = 0; i < elems.length; i++) {
    ...
    if (isWhite(fill) && i <= lastWhiteIdx) { koEls.push(setFill(raw, "black")); continue }
    if (!isWhite(fill)) {
      const el = setFill(raw, "currentColor")
      if (i <= lastWhiteIdx) mainEls.push(el); else overEls.push(el)
    }
  }
...
interface MatrixGroupInfo { gAttrs: string; inner: string; firstFill: string }
...
function scanMatrixGroups(svg: string): {
  ...
  for (const g of groups) {
    ...
    if (isNone(fv)) { textGroups.push(g); continue }
    if (isWhite(fv)) { badgeGroups.push(g); continue }
    if (isGradient(fv)) continue
    if (!boxGroup) { boxGroup = g } else return null  // multiple boxes → ambiguous
  }
...
function buildMatrixBadge(svg: string, groups: NonNullable<ReturnType<typeof scanMatrixGroups>>): string {
  ...
  const textStr = groups.textGroups.map(g => {
    const inner = g.inner.replace(
      /<(path|circle|rect|ellipse)\b(?![^>]*\bfill\s*=)/gi,
      '<$1 fill="currentColor"',
    )
    return `<g${g.gAttrs}>${inner}</g>`
  }).join("\n  ")
...
function removeFullBleed(svg: string, vw: number, vh: number): string {
  ...
  svg = svg.replace(/<rect([^>]*)(?:\/>|>[\s\S]*?<\/rect>)/gi, (m, a) => {
    ...
    const w = dim("width", vw), h = dim("height", vh)
  ...
  let body = svg.replace(/<defs[\s\S]*?<\/defs>/gi, m => {
    defsStash.push(m); return `\x00DEFS${defsStash.length - 1}\x00`
  })
  ...
  body = body.replace(/<path([^>]*)(?:\/>|>[\s\S]*?<\/path>)/gi, (m, a) => {
    ...
    if (isBoxPath(d, vw, vh)) { removedBox = true; return "" }
    return m
  })
  svg = body.replace(/\x00DEFS(\d+)\x00/g, (_, i) => defsStash[Number(i)] ?? "")
...
function simpleMono(svg: string): string {
  ...
  const withDefsStripped = svg.replace(/<defs[\s\S]*?<\/defs>/gi, m => {
    defsParts.push(m); return `\x00DEFS${defsParts.length - 1}\x00`
  })
  const withNoFillFixed = svgRootHasNoneFill ? withDefsStripped : withDefsStripped.replace(
    /<(path|circle|rect|ellipse|polygon|polyline)\b(?![^>]*\bfill\s*=)([^>]*(?:\/>|>))/gi,
    '<$1 fill="currentColor"$2',
  )
  svg = withNoFillFixed.replace(/\x00DEFS(\d+)\x00/g, (_, i) => defsParts[Number(i)])
...
async function run() {
  ...
  const slugs =
    ...
    force ? await getAllWithSource() :
    await getMissingMono()

  console.log(`Processing ${slugs.length} icons...\n`)

  let added = 0, skipped = 0, knockout = 0

  for (const slug of slugs) {
    const dir = join(ASSETS_DIR, slug)
    let files: string[]
    try { files = await readdir(dir) } catch { skipped++; continue }

    if (files.includes("mono.svg") && !force) { skipped++; continue }

    const hasPlan  = files.includes("plain.svg")
    const hasOrig  = files.includes("original.svg")
    if (!hasPlan && !hasOrig) { skipped++; continue }
    ...
    if (!isKo && hasPlan && hasOrig) {
      ...
      if (origMono.includes('id="ko"') || origMono.includes('id="wko"')) {
        src = "original.svg"; raw = origRaw; mono = origMono; isKo = true
      }
    }
